...
const DepthCarousel = ({
  ...
  // Vertical variant: cards stack along Y (bottom -> up) and tilt on the X axis,
  // instead of the original horizontal X-spread / rotateY tilt.
  const layout = useCallback((pos: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const ty = dir * cfg.spread * d;
      const rx = dir * cfg.tilt * clamp(d, 0, 1);
      ...
      el.style.transform = `translate(-50%, -50%) scale(${sc}) translateY(${ty.toFixed(2)}px) translateZ(${tz.toFixed(2)}px) rotateX(${rx.toFixed(3)}deg)`;
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const cfg = cfgRef.current;
      const needed = cfg.cardHeight + Math.abs(cfg.spread) * 2 + 120;
      scaleRef.current = clamp(h / needed, 0.4, 1);
      layout(posRef.current);
    });
    ro.observe(root);
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      const step = clamp(delta / (cfg.cardHeight * 0.9), -0.6, 0.6);
      posRef.current += step;
      layout(posRef.current);
  ...
  const onPointerMove = useCallback(
    (e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      if (!drag) return;
      const cfg = cfgRef.current;
      const stepPx = Math.max(cfg.cardHeight * 0.55 * scaleRef.current, 40);
      ...
      drag.lastT = now;
      posRef.current = drag.startPos + dy / stepPx;
      layout(posRef.current);
    },
    [layout]
  );
    ...
    >
      <div className="absolute inset-0 [transform-style:preserve-3d]" ref={stageRef}>
        {data.map((item, i) => (
          ...
          >
            {/* eslint-disable-next-line @next/next/no-img-element -- carousel renders a dynamic, user-provided image list; next/image's fixed intrinsic sizing doesn't fit this transform-driven layout */}
            <img
              className="block h-full w-full select-none object-contain [pointer-events:none] [-webkit-user-drag:none]"
              src={item.image}
              alt={item.alt || ""}
              draggable={false}
            />
          ...
          {data.map((_, i) => (
            <button
              ...
              className={`w-[7px] cursor-pointer rounded-full transition-[height,background] duration-[250ms] ${
                active === i ? "h-5 bg-[#EA580C]" : "h-[7px] bg-white/30"
              }`}
