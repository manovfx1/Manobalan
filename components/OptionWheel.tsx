...
const EMPTY_CONFIG: WheelConfig = {
  count: 0,
  items: [],
  rowH: 1,
  curve: 1,
...
export default function OptionWheel({
  ...
  const cfgRef = useRef<WheelConfig>(EMPTY_CONFIG);
  ...
  // Keep the latest callback and layout config on refs so the stable rAF
  // loop and event handlers below always read current values without
  // needing to be recreated (and re-subscribed) on every prop change. Runs
  // as a layout effect (no deps, so every render) rather than inline during
  ...
  const runFrame = useCallback((now: number) => {
    ...
    // eslint-disable-next-line react-hooks/immutability -- self-referencing rAF loop; runFrame's identity never changes (empty deps), so this always calls the current closure
