...
import { useRef, useEffect, useMemo, forwardRef, type ForwardRefExoticComponent, type RefAttributes } from "react";
...
type RetroEffectComponentProps = { colorNum: number; pixelSize: number } & RefAttributes<RetroEffectImpl>;
const WrappedRetroEffect = wrapEffect(RetroEffectImpl) as unknown as ForwardRefExoticComponent<RetroEffectComponentProps>;
...
function DitheredWaves({
  ...
  const waveUniforms = useMemo<WaveUniforms>(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- uniforms object must keep a stable identity for the shaderMaterial; values are pushed in via useFrame below instead of dependency-driven recreation.
    []
  );
  ...
  /* eslint-disable react-hooks/immutability -- three.js shader uniforms are mutated in place every frame by design; this is the standard r3f pattern for pushing animated values to the GPU. */
  useFrame(({ clock }) => {
